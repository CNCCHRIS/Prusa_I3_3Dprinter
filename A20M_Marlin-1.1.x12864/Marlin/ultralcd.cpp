   ...
   if (encoderPosition != 0)
    {
	 ...
	 if(mixer.rate[NOZZLE0] >112)
      ...
      //color_change_flag =0;
      u8g.setPrintPos(10, 24);//lcd.setCursor(2,1);
      lcd_print(tmp);//   lcd.print(tmp);
...
//////////
  ...
  sprintf(tmp,"start percent %d%% ", mixer.min); 	
  u8g.setPrintPos(0, 24);//lcd.setCursor(1,1);
  lcd_print(tmp);//lcd.print(tmp);
  MENU_ITEM(submenu, "", lcd_mixer_endPrecent_menu);
  sprintf(tmp,"end percent %d%% ", mixer.max); 	
  u8g.setPrintPos(0, 36);//lcd.setCursor(1,2);
  lcd_print(tmp);//lcd.print(tmp);
  END_MENU();
}
...
static void lcd_mixer_menu()
{
	...
	char tmp[32];
	u8g.setPrintPos(90, 24);//lcd.setCursor(15,1);
	sprintf(tmp,"%d%%",mixer.rate[NOZZLE0]);
	lcd_print(tmp);//lcd.print(tmp);

	sprintf(tmp,"%d%%",mixer.rate[NOZZLE1]);
	u8g.setPrintPos(90, 36);//lcd.setCursor(15,2);
	lcd_print(tmp);//lcd.print(tmp);
...
/**************************************liu************************************************/
  ...
  void lcd_control_filament_menu() {
    ...
    MENU_ITEM_EDIT(bool, MSG_FILAMENT_RUNOUT, &filament_runout_enabled);
